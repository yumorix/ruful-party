import { Participant, Vote } from '../db/supabase';
...
export async function generateMatches(
  ...
  for (const vote of votes) {
    ...
    if (votedParticipant.gender === 'female') {
      femaleVoteCount.set(votedParticipant.id, (femaleVoteCount.get(votedParticipant.id) || 0) + 1);
    } else {
      maleVoteCount.set(votedParticipant.id, (maleVoteCount.get(votedParticipant.id) || 0) + 1);
    }
  }

  // Sort by vote count (descending)
  remainingMales.sort((a, b) => (maleVoteCount.get(b.id) || 0) - (maleVoteCount.get(a.id) || 0));

  remainingFemales.sort(
    (a, b) => (femaleVoteCount.get(b.id) || 0) - (femaleVoteCount.get(a.id) || 0)
  );
  ...
  const unmatched = participants.filter(p => !allMatchedParticipants.has(p.id)).map(p => p.id);
