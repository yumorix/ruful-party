...
export async function POST(request: NextRequest) {
  try {
    ...
    if (party.status !== 'active') {
      return NextResponse.json({ error: 'Party is not active' }, { status: 403 });
    }
    ...
    // Validate vote data
    if (
      !Array.isArray(body.voted_ids) ||
      body.voted_ids.length === 0 ||
      body.voted_ids.length > 3
    ) {
      return NextResponse.json({ error: 'You must vote for 1-3 participants' }, { status: 400 });
    }
