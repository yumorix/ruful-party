...
// Match queries
export async function getMatches(partyId: string, matchType: 'interim' | 'final'): Promise<Match[]> {
  const { data, error } = await supabase
    .from('matches')
    .select('*')
    .eq('party_id', partyId)
...
export async function getSeatingPlan(partyId: string, planType: 'interim' | 'final'): Promise<SeatingPlan | null> {
  const { data, error } = await supabase
    .from('seating_plans')
    .select('*')
    .eq('party_id', partyId)
